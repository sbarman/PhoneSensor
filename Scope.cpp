...
int main(int argc, char** argv) {
  if (!init_graphics()) exit(-1);
  if (!init_sound()) exit(-1);
...
//fprintf(jlog,"%d,%d,%d (raw input other channel)\n",iteration,(int)i,(int)(((signed short*)data)[2*i+1]) );
    ...
    fprintf(stderr,"chan1avg: %f  max:%d  min:%d    chan2avg:%f\n",accum1/frames,(int)maxval,(int)minval,accum2/frames);
      ...
      for(int i=1; i<frames; i++)
	{
	  if(in[i]<threshold&&in[i-1]<threshold)
	    seenlessthresh=i;
	  if(in[i]>=threshold && seenlessthresh!=-1 && i-seenlessthresh<10) //J: for positive thresh value, this gives rising edge
	    {
	      start=i;
	      break;
	    }
	...
	for(int c=1; c< end; c++)
	  {
		...
		if(vlast<=v)
		  {
		    ((char*) screen->pixels)[DIM_sw * r + c] = (vlast<=r && r<=v) ? 200 : 0;		    		  }

		if(vlast>v)
		  {
		    ((char*) screen->pixels)[DIM_sw * r + c] = (v<=r && r<=vlast) ? 200 : 0;		    		  }
