...
int sin_start(int argc, char *argv[], fnc_info * function_info) {
	struct option long_option[] = { { "help", 0, NULL, 'h' }, { "device", 1,
			...
			NULL, 'p' }, { "method", 1, NULL, 'm' }, { "format", 1, NULL, 'o' }, {
			"verbose", 1, NULL, 'v' }, { "noresample", 1, NULL, 'n' }, { "pevent", 1,
			NULL, 'e' }, { NULL, 0, NULL, 0 }, };
	snd_pcm_t *handle;
	int err, morehelp;
	...
	while (1) {
		int c;
		if ((c = getopt_long(argc, argv, "hD:r:c:f:b:p:m:o:vne", long_option, NULL))
				< 0)
			break;
		switch (c) {
		case 'h':
